...
def _parse_n8n_ocr_response(body: str) -> dict:
    ...
    # Parse JSON thường / array trước (trường hợp webhook không streaming)
    if len(lines) == 1:
        try:
...
def _extract_ocr_fields(data: dict) -> dict:
    ...
    # n8n 'parse output to json' node trả về: amount, vendor, date, suggested_category, note, line_items, confidence, needs_review, warnings.
    def _get(*keys):
        for k in keys:
            v = data.get(k)
            if v is not None:
...
async def _parse_receipt_via_n8n(
    ...
    # Gửi ảnh hóa đơn lên n8n OCR webhook dưới dạng multipart/form-data. n8n nhận file binary, xử lý OCR, trả về JSON kết quả.
